...
/** PrimeIcons 7 has no rain/wind glyphs — render matching outline SVGs instead. */

type WeatherIconKind = 'pi' | 'rain' | 'wind';
...
export class TripProposalsPage implements OnInit {
  ...
  protected hasCostBreakdown(proposal: TripProposal): boolean {

    return !!proposal.costBreakdown?.estimatedTotal;

  }



  protected formatCostRange(range: CostRange, currency: string): string {
  ...
  protected formatTotalRange(breakdown: ProposalCostBreakdown): string {

    const { min, max, currency } = breakdown.estimatedTotal;
  ...
  protected showTransportSubCosts(transport: TransportCostBreakdown): boolean {

    return !!transport.includesCarCosts && !!transport.sub;

  }



  protected hasSubCost(range: CostRange | null | undefined): boolean {
  ...
  protected weatherIconKind(icon: string): WeatherIconKind {
